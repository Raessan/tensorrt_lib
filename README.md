...
If you face compilation issues, it is possible that the TensorRT library does not match the expected by the library. There are methods, such as `enqueueV3` in `tensorrt_engine.cpp` or `execute_async_v3` in `tensorrt_engine.py` that may not be compatible with your TensorRT version. Also, remember that C++17 is needed to compile (although it can be downgraded if the dependency to `filesystem` is removed).

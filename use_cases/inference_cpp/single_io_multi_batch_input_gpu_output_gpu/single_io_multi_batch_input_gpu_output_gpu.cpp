...
int main(){
    ...
    // Perform WARMUP inference (only with the first batch). We set from_device=true and to_device=true
    for (int i=0; i< n_inferences_warmup; i++){
        nn_handler.run_inference(input_gpu, output_pred_gpu, true, true);
    }
    
    // Get the current time before inference. We set from_device=true and to_device=true
